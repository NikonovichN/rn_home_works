...
describe('Search product card', () => {
  ...
  test('test component', async () => {
    ...
    render(<SearchProductCard onPress={onPressCard} product={mockProduct} />);

    const findName = await screen.findByText(mockProduct.name);
    const findDescription = await screen.findByText(mockProduct.description);
    ...
    expect(findDescription).toBeDefined();
